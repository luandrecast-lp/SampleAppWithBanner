...
class MessagingViewController: UIViewController {
    ...
    private var conversationContainerVC: ConversationContainerViewController?
    ...
    private let authenticationType: LPAuthenticationType = .authenticated
    ...
    @IBAction func showConversation() {
        ...
        if windowSwitchValue {
            self.conversationContainerVC = nil
        } else {
            //for ViewController Mode ONLY
            let storyboard = UIStoryboard(name: "Main", bundle: nil)
            let controller = storyboard.instantiateViewController(withIdentifier: "ConversationContainerViewController")
            guard let convoViewController = controller as? ConversationContainerViewController else { return }
            self.conversationContainerVC = convoViewController
        }
        ...
        if let vc = self.conversationContainerVC {
...
extension MessagingViewController {
    ...
    private func showConversationFor(accountNumber: String, authenticatedMode: Bool) {
        ...
        if self.conversationContainerVC != nil {
            self.conversationContainerVC?.accountNumber = accountNumber
            self.conversationContainerVC?.conversationQueryProtocol = conversationQuery
        }
        
        //LPConversationViewParams
        let conversationViewParams = LPConversationViewParams(conversationQuery: conversationQuery,
                                                              containerViewController: self.conversationContainerVC?.conversationViewController,
                                                              isViewOnly: false,
    ...
    private func logoutLPSDK() {
        LPMessaging.instance.logout(completion: {
            ...
            print("failed to logout from MessagingSDK - error: \(error.first?.localizedDescription)")
        }
    }
}
...
extension MessagingViewController: LPMessagingSDKdelegate {
    ...
    */
    func LPMessagingSDKAgentDetails(_ agent: LPUser?) {
        guard self.conversationContainerVC != nil else { return }

        let name: String = agent?.nickName ?? ""
        self.conversationContainerVC?.title = name
    }
